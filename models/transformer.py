...
class BackboneBase(nn.Module):
    ...
    def __init__(self, backbone: nn.Module, train_backbone: bool, return_interm_layers: bool = True):
        super().__init__()
...
if __name__ == '__main__':
    ...
    w = 8
    featuresize = 2048
    backbone = ResNetBackbone('resnet50', train_backbone=True)
    input_proj = nn.Conv2d(backbone.num_channels[-1], featuresize, 1)
    tModel = Transformer(d_model=featuresize)
    posEmbed = PositionEmbeddingLearned(64, 64, num_pos_feats=featuresize//2)
    queryEmbed = nn.Embedding(16, featuresize)
    classEmbed = nn.Linear(featuresize*2, 17)
    kptEmbed = MLP(featuresize*2, featuresize//4, 2, 3)
    
    extractor = Joiner(backbone, posEmbed)

    #print('qe',queryEmbedding.size())
    src = torch.rand((b, c, 64, 64))

    srcfeature, pos = extractor(src)
    hori = tModel(input_proj(srcfeature[-1]), None, queryEmbed.weight, pos[-1])[0]
    vert = tModel(input_proj(srcfeature[-1]), None, queryEmbed.weight, pos[-1])[0]

    logits = classEmbed(torch.cat((hori[-1], vert[-1]), 2))
    kpts = kptEmbed(torch.cat((hori[-1], vert[-1]), 2)).sigmoid()

    print(logits.size(), kpts.size())